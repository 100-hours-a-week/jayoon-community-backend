...
/**
 * Controller 내에서 발생하는 모든 에러를 핸들링 합니다. Controller 레벨에서 별도로 try-catch문이 없기 때문에 해당 레이어 이후 단계에서 발생한 모든
 * 예외를 여기에서 처리하게 됩니다.
 *
 * Notes Controller 레이어 보다 이전에서 예외가 발생하면? 예) 인증, 인가 등 아직 제대로 다뤄보진 않았지만 Filter 레벨에서 발생하는 에러는 이후에 구현할
 * 예정입니다.
 */
...
public class GlobalExceptionHandler {
     ...
     *
     * 위의 핸들러 메서드에서 catch 되지 않은 모든 예외들이 여기에서 처리됩니다. 서버에서 개발자가 직접 다루지 못한 예외이므로 해당 예외 객체 ex는
     * ErrorStatusCodeProvider를 구현하지 않았습니다.
     */
